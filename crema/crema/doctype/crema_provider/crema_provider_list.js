		...
		enabled(value, df, doc) {
			if (!doc.enabled) {
				if (doc.auto_disabled) {
					// last_check_detail and last_checked hold provider text — escape both.
					const label = doc.last_check_detail
						? `${__("Auto-disabled")}: ${doc.last_check_detail}`
						: __("Auto-disabled");
					return `<span class="indicator-pill orange" title="${frappe.utils.escape_html(
						doc.last_checked || ""
					)}">${frappe.utils.escape_html(label)}</span>`;
				}
				return `<span class="indicator-pill gray">${__("Disabled")}</span>`;
			}
